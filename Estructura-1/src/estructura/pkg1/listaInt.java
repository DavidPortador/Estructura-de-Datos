...
public class listaInt {
    ...
    public listaInt(classInt p_int) {
        Nodo v_nodo = new Nodo(p_int);
        v_nodo.insertarPrincipio(p_int);
        ...
        while (true) {
            if (a_longitud < 1) {
                ...
                System.out.println("\nCabeza: " + v_nodo.obtener(0).getA_entero());
                if (a_longitud == 1)
                    System.out.println("Cola: null");
                else
                    System.out.println("Cola: " + v_nodo.obtener(a_longitud - 1).getA_entero());
            }
        ...
        switch (v_opc) {
            case 1:
                ...
                if (a_longitud < 1)
                    ...
                    System.out.println("Solo exite el dato [" + p_nodo.obtener(0).getA_entero() + "]");
                else {
                    System.out.println("Cabeza(0) a Cola(" + (a_longitud - 1) + ")");
                    while (v_dato < 0 || v_dato >= a_longitud) 
                        v_dato = valiEntero("Obtener dato: ");
                    System.out.println("Dato (" + v_dato + ") tiene = " + "[" + p_nodo.obtener(v_dato).getA_entero() + "]");
                }
                break;
            case 2:
                int v_cont;
                ...
                for (v_cont = 0; v_cont < a_longitud; v_cont++) {
                    System.out.print(p_nodo.obtener(v_cont).getA_entero());
            ...
            case 3:
                ...
                v_val = valiEntero("Dato: ");
                classInt v_inInicio = new classInt(v_val);
                p_nodo.insertarPrincipio(v_inInicio);
                break;
            case 4:
                a_tec.nextLine(); // Limpia el buffer para la entrada de datos
                v_val = valiEntero("Dato: ");
                classInt v_inFinal = new classInt(v_val);
            ...
            case 5:
                ...
                if (a_longitud == 1) {
                    ...
                    v_val = valiEntero("Dato");
                    classInt v_xn = new classInt(v_val);
                    if (v_op == 0) {
    ...
    private class Nodo {
        ...
        public classInt a_ent;
        // Constructor que recibe el objeto a manipular
        public Nodo(classInt p_int) {
            a_ent = p_int;
        }

        void insertarPrincipio(classInt p_int) {
        ...
        void insertarFinal(classInt p_int) {
            Nodo v_nuevoNodo = new Nodo(p_int);
        ...
        void insertarXn(int p_num, classInt p_int) { // Posicion especifica
            Nodo v_nuevoNodo = new Nodo(p_int);
            // Valida que la lista no este vacia
            if (a_cabeza == null) {
                a_cabeza = v_nuevoNodo;
